...
const CONFIG = {
  ...
  app: {
    ...
    routesToCheck: ['/', '/api/health', '/api/auth/me'],
...
function startApplication() {
  ...
  // Kill any existing process with a promise
  const killExisting = () => {
  ...
  killExisting().then(() => {
    ...
    log(`Application started with PID: ${appProcess.pid}`, 'info');
...
function keepConnectionAlive() {
  STATE.pingCount++;
  log(`Sending self-ping to keep application alive...`, 'debug');
  
  // Try to ping the server
  http.get(CONFIG.app.internalUrl, (res) => {
    log('Received health check ping', 'debug');
    res.resume();
    log('Self-ping successful', 'debug');
  }).on('error', (err) => {
    log(`Self-ping failed: ${err.message}`, 'warn');
    checkAndRestartIfNeeded();
  });
  
  // Also perform a full health check periodically
  if (STATE.pingCount % 5 === 0) {
    performHealthCheck().then(isHealthy => {
      if (!isHealthy && !STATE.isRestarting) {
        log('Periodic health check failed, triggering verification...', 'warn');
        checkAndRestartIfNeeded();
      }
    });
  }
...
const monitorServer = http.createServer((req, res) => {
    ...
    const html = `
      ...
      <html>
        ...
        <body>
          ...
          <div class="actions">
            <button onclick="fetch('/restart').then(() => window.location.reload())">Restart Application</button>
  ...
  // Ping endpoint (for external services to ping)
  else if (path === '/ping') {
...
function initialize() {
  log('Never-Sleep system initializing...', 'info');
  
  // Start the monitoring server
  monitorServer.listen(CONFIG.monitor.port, '0.0.0.0', () => {
    log(`Monitoring server listening on port ${CONFIG.monitor.port}`, 'info');
    log(`Dashboard available at: http://localhost:${CONFIG.monitor.port}/`, 'info');
    
    // Start the application
    startApplication();
    
    // Set up keep-alive pings
    setInterval(keepConnectionAlive, CONFIG.monitor.pingInterval);
    
    // Set up regular health checks
    setInterval(checkAndRestartIfNeeded, CONFIG.monitor.checkInterval);
  });
  
  // Handle server errors
  monitorServer.on('error', (err) => {
    log(`Monitoring server error: ${err.message}`, 'error');
    
    // If the port is in use, try a different one
    if (err.code === 'EADDRINUSE') {
      CONFIG.monitor.port++;
      log(`Port in use, trying ${CONFIG.monitor.port} instead`, 'warn');
      setTimeout(() => {
        monitorServer.listen(CONFIG.monitor.port, '0.0.0.0');
      }, 1000);
    }
  ...
  log('Never-Sleep system initialized successfully', 'info');
}

// Graceful shutdown
function gracefulShutdown() {
  log('Graceful shutdown initiated', 'info');
  ...
  if (STATE.appProcess) {
    try {
      ...
      log(`Failed to kill application process: ${e.message}`, 'error');
    }
  }
  ...
  log('Never-Sleep system shut down successfully', 'info');
  process.exit(0);
}
