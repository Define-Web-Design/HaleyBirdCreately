...
export const LegalAcceptanceModal = ({
  ...
  const handleSkip = () => {
    // Record that user has seen but skipped the terms
    const skipEvent = new CustomEvent('legalAction', {
      detail: {
        action: 'skip',
        documentType,
        version
    ...
    toast({
      ...
      description: "You can access the features while deciding on the terms later.",
    });
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="flex items-center gap-1">
            ...
            <span>Transparency is important to us. Here's how we handle your data.</span>
          </DialogDescription>
        </DialogHeader>
        
        <div className="mb-4 bg-muted/20 rounded-lg p-4 border border-primary/10">
          <div className="flex gap-2 items-start">
            <Info className="h-5 w-5 text-primary mt-0.5" />
            <div>
              <h3 className="text-sm font-medium mb-1">You Have Options</h3>
              <p className="text-sm text-muted-foreground">
                We believe in transparency. You can use {requiredForFeature} without accepting terms,
                but we want you to understand how your data is used.
              </p>
        ...
        {showDetails && (
          <div className="max-h-[40vh] overflow-y-auto border rounded-md p-4 mb-4">
            {documentType === 'terms' ? (
        ...
        <div className="space-y-4">
          <div className="border-t border-border pt-4">
            <h3 className="text-sm font-medium mb-2">Your Data Privacy Choices</h3>
            <div className="flex flex-col gap-3 bg-muted/10 p-3 rounded-md">
              <div className="flex justify-between items-center">
                <div className="text-sm">Access to {requiredForFeature}</div>
                <div className="text-xs text-primary bg-primary/10 px-2 py-0.5 rounded-full">Always Allowed</div>
              </div>
              
              <div className="flex justify-between items-center">
                <div className="text-sm">Data collection for improving service</div>
                <Switch id="data-collection" />
              </div>
              
              <div className="flex justify-between items-center">
                <div className="text-sm">Marketing communications</div>
                <Switch id="marketing" />
              </div>
            </div>
          </div>
        ...
        <DialogFooter className="flex flex-col sm:flex-row gap-2 sm:gap-0">
          ...
          <Button 
            ...
            className="w-full sm:w-auto siri-glow order-1 sm:order-2"
          >
