...
export default function TopNavigation({ toggleSidebar, isMobile }: { toggleSidebar: () => void, isMobile?: boolean }) {
  ...
  // Enhanced mobile view with improved accessibility and touch targets
  const renderMobileView = () => {
    if (showMobileSearch) {
      return (
        <header className="bg-background/95 backdrop-blur-sm border-b border-border flex items-center justify-between px-3 sm:px-5 py-2.5 h-16 sticky top-0 z-50 transition-all duration-300 ease-in-out">
          <div className="flex items-center w-full">
            <button 
              onClick={() => setShowMobileSearch(false)}
              className="p-3 rounded-md mr-2 hover:bg-accent/80 focus:ring-2 focus:ring-primary focus:outline-none active:bg-accent"
    ...
    return (
      <header className="bg-background/95 backdrop-blur-sm border-b border-border flex items-center justify-between px-3 sm:px-5 py-2.5 h-16 sticky top-0 z-50 transition-all duration-300 ease-in-out">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          <button
            className="p-3 rounded-md hover:bg-accent/80 transition-colors duration-200 focus:ring-2 focus:ring-primary focus:outline-none active:bg-accent"
        ...
        {/* Right section with actions - larger touch targets for mobile */}
        <div className="flex items-center gap-0.5 sm:gap-2">
          ...
          <Link href="/profile" className="group focus:outline-none p-1.5 rounded-md focus:ring-2 focus:ring-primary">
            <div className="w-9 h-9 overflow-hidden rounded-full bg-muted flex items-center justify-center hover:ring-2 hover:ring-primary/50 transition-all duration-200 shadow-sm active:scale-95">
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        {notificationsOpen && (
          <>
            <div className="fixed inset-0 bg-black/20 z-40 touch-none animate-in fade-in" onClick={() => setNotificationsOpen(false)}></div>
            <div className="fixed inset-x-0 top-16 max-h-[calc(100vh-4rem)] sm:absolute sm:right-4 sm:top-16 sm:left-auto w-full sm:w-80 max-w-[100vw] sm:max-w-[360px] bg-background border rounded-b-lg sm:rounded-lg shadow-lg z-50 overflow-y-auto animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="flex items-center justify-between px-4 py-3 border-b">
                <h3 className="font-medium">Notifications</h3>
                <button 
                  onClick={() => setNotificationsOpen(false)}
                  className="p-2 rounded-md hover:bg-accent/80 text-muted-foreground hover:text-foreground active:bg-accent"
              ...
              <div className="p-4">
                <div className="space-y-3">
                  <div className="p-3 bg-muted/50 rounded-md border hover:border-border/80 transition-colors cursor-pointer active:bg-muted">
                ...
                <button className="w-full mt-4 py-2 text-center text-sm text-primary hover:underline active:text-primary/80">
  ...
  // Enhanced desktop view with better touch targets and accessibility
  const renderDesktopView = () => (
    <header className="bg-background/95 backdrop-blur-sm border-b border-border flex items-center justify-between px-5 py-2.5 h-16 sticky top-0 z-50 transition-all duration-300 ease-in-out">
      ...
      <div className="flex items-center gap-2">
        <button
          className="mr-2 p-3 rounded-md hover:bg-accent/80 transition-colors duration-200 focus:ring-2 focus:ring-primary focus:outline-none active:bg-accent"
          onClick={toggleSidebar}
          aria-label="Toggle sidebar"
          aria-expanded={sidebarOpen}
        >
      ...
      <div className="max-w-xl w-full mx-5">
        <div className="relative">
          ...
          {searchTerm && (
            <button 
              className="absolute right-3 top-1/2 transform -translate-y-1/2 text-muted-foreground hover:text-foreground p-1.5 rounded-md focus:outline-none"
              onClick={() => setSearchTerm('')}
              aria-label="Clear search"
            >
      ...
      <div className="flex items-center gap-2">
        ...
        <Popover>
          ...
          <PopoverContent className="w-80 p-0">
            ...
            <div className="space-y-3 p-4">
              <div className="p-3 bg-muted/50 rounded-md hover:bg-muted/80 transition-colors cursor-pointer">
                <div className="flex items-start">
                  <div className="bg-primary/10 text-primary rounded-full p-2 mr-3 shrink-0">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <path d="M9 5H2v7l6.29 6.29c.94.94 2.48.94 3.42 0l6.29-6.29c.94-.94.94-2.48 0-3.42L11.71 2.71c-.94-.94-2.48-.94-3.42 0L5 6"></path>
            ...
            <button className="w-full p-3 border-t text-center text-sm text-primary hover:bg-accent/40 hover:underline">
              View all notifications
            </button>
          </PopoverContent>
        </Popover>

        <Button variant="outline" size="icon" className="ml-1 relative" asChild>
          <Link href="/settings">
            <Settings size={18} />
        ...
        <Link 
          href="/content-library" 
          className="ml-1 flex items-center gap-2 px-4 py-2 rounded-md bg-gradient-to-r from-[#F2994A] to-[#FF9DAE] text-white hover:opacity-90 transition-all duration-300 ease-in-out focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:outline-none shadow-sm active:opacity-80 hover:shadow-md"
        >
          <span>Create</span>
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <line x1="12" y1="5" x2="12" y2="19"></line>
        ...
        <Link href="/profile" className="ml-1 focus:outline-none">
          <div className="w-9 h-9 overflow-hidden rounded-full bg-muted flex items-center justify-center hover:ring-2 hover:ring-primary transition-all duration-200 focus:ring-2 focus:ring-primary shadow-sm active:scale-95">
            <User size={18} />
          </div>
        </Link>
      </div>
